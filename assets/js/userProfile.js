document.addEventListener("DOMContentLoaded", () => {
  ...
  const userProfileChosen = twittUsers.find(
    (user) =>
      user.username.toLowerCase() === usernameProfileChosen.toLowerCase()
  );
  ...
  const userProfileTwitts = existingTwitts.filter(
    (twitt) => twitt.twittUsernameOwner === usernameProfileChosen
  );

  function displayAllTwitt(twitts = userProfileTwitts) {
    if (twitts.length === 0) {
      ...
      twitts.forEach((twitt) => {
        ...
        const hasLiked = twittManager.userHasLikeTwittValidate(
          twitt.id,
          usernameLoggedIn
        );
        ...
        itemTwitt.innerHTML = `
              <div class="flex items-center justify-between">
                  <div class="flex items-center justify-start">
                      <img id="visitProfile-${ownerTwitt.username}" src="${
          ownerTwitt.avatar
            ...
            if (result.success) {
              displayAllTwitt(
                twittManager
                  ...
                  .filter(
                    (twitt) =>
                      twitt.twittUsernameOwner === usernameProfileChosen
                  )
              );
            } else {
              instantFeedback.style.display = "flex";
